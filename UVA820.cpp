...
//se utilizo algoritmo Dinic de la pagina geekforgeeks
...
int main(){
	...
	while(N!=0){
		...
		for (itr=con.begin(); itr!=con.end(); ++itr){
			g.addEdge(itr->first.first,itr->first.second,itr->second);
		}
		cout<<"Network "<<caseN<<endl;
 		cout << "The bandwidth is " << g.DinicMaxflow(s-1, t-1)<<"."<<endl<<endl;
		cin>>N;
	}
